...
class ImageSectionValueObject
{
    ...
    public function getImageAtFramePosition(int $framePosition, VideoSizeEnum $videoSize): ImageInterface
    {
        ...
        $image->crop($videoSize->getWidth() * ImageService::OVERSIZE_QUALITY_FACTOR, $videoSize->getHeight() * ImageService::OVERSIZE_QUALITY_FACTOR, $left, $top);
    ...
    {
        $left = ($this->image->width() - ($videoSize->getWidth() * ImageService::OVERSIZE_QUALITY_FACTOR)) / 2;
        $top = ($this->image->height() - ($videoSize->getHeight() * ImageService::OVERSIZE_QUALITY_FACTOR)) / 2;
        return [$left, $top];
    }
    ...
    private function getLeftTopValuesForTopLeftToBottomRight(VideoSizeEnum $videoSize, int $framePosition): array
    {
        ...
        $deltaX = $this->image->width() - ($videoSize->getWidth() * ImageService::OVERSIZE_QUALITY_FACTOR);
        $deltaY = $this->image->height() - ($videoSize->getHeight() * ImageService::OVERSIZE_QUALITY_FACTOR);
    ...
    private function getValuesForCenterZoomOut(VideoSizeEnum $videoSize, int $framePosition)
    {
        ...
        $left = ($width - ($videoSize->getWidth() * ImageService::OVERSIZE_QUALITY_FACTOR)) / 2;
        $top = ($height - ($videoSize->getHeight() * ImageService::OVERSIZE_QUALITY_FACTOR)) / 2;
    ...
    private function getValuesForCenterZoomIn(VideoSizeEnum $videoSize, int $framePosition)
    {
        ...
        $deltaY = $this->image->height() - ($videoSize->getHeight() * ImageService::OVERSIZE_QUALITY_FACTOR);
        $factor = 100 / $totalFrames;
        $width = ($videoSize->getWidth() * ImageService::OVERSIZE_QUALITY_FACTOR) + $deltaX * ($factor * $framePosition) / 100;
        $height = ($videoSize->getHeight() * ImageService::OVERSIZE_QUALITY_FACTOR) + $deltaY * ($factor * $framePosition) / 100;
        $left = ($width - ($videoSize->getWidth() * ImageService::OVERSIZE_QUALITY_FACTOR)) / 2;
        $top = ($height - ($videoSize->getHeight() * ImageService::OVERSIZE_QUALITY_FACTOR)) / 2;
